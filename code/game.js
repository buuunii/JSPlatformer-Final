...
function Lala(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2,0.1));
  this.size = new Vector(0.95, 1.4);
  this.wobble = Math.random() * Math.PI * 2;
}
Lala.prototype.type = 'lala';
...
var wobbleSpeed = 5;
var wobbleDist = 0.07;
Lala.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Slow.prototype.act = function(step) {
  this.wobble += step * slowSpeed;
  var wobblePos = Math.sin(this.wobble) * slowDist;
  this.pos = this.basePos.plus(new Vector(wobblePos, 0));
};
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  if (obstacle) {
    if (obstacle == "portal") {
      this.pos = this.pos.plus(new Vector (-5, -5));
    }
    level.playerTouched(obstacle);
